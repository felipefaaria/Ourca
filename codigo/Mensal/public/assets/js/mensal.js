...
function getEvents() {
  ...
  if (storedEvents) {
    ...
    // Fetch from the server if not found in local storage
    fetch("/events")
      .then((response) => {
        /* ... */
      })
      .then((data) => {
        eventsArr.push(...data);
        localStorage.setItem("events", JSON.stringify(eventsArr)); // Save to local storage
        initCalendar();
      })
      .catch((error) => console.error("Error fetching events:", error)); // ... rest of your fetch logic
  }
}

// Função para salvar eventos no servidor
function saveEvents() {
  fetch("/events", {
    method: "POST",
    headers: {
...
document.addEventListener("DOMContentLoaded", () => {
  const links = document.querySelectorAll("#hrefsono");
  links.forEach((link) => {
